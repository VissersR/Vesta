...
public class MinifigInputManager : MonoBehaviour
    {
        protected Unity.LEGO.Minifig.MinifigController m_MinifigController;
        GameFlowManager m_GameFlowManager;

        protected virtual void Awake()
        {
            m_MinifigController = GetComponent<Unity.LEGO.Minifig.MinifigController>();
        ...
        protected virtual void OnGameOver(GameOverEvent evt)
        {
            ...
            if (evt.Win)
            {
                ...
                switch (randomCelebration)
                {
                    case 0:
                        {
                            m_MinifigController.PlaySpecialAnimation(Unity.LEGO.Minifig.MinifigController.SpecialAnimation.AirGuitar);
                            break;
                        }
                    case 1:
                        {
                            m_MinifigController.PlaySpecialAnimation(Unity.LEGO.Minifig.MinifigController.SpecialAnimation.Flexing);
                            break;
                        }
                    case 2:
                        {
                            m_MinifigController.PlaySpecialAnimation(Unity.LEGO.Minifig.MinifigController.SpecialAnimation.Dance);
                            break;
                        }
                }
            }
